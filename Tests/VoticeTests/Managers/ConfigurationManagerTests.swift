...
extension ConfigurationManagerTests {
  ...
  func testOptionalVisibleStatusCombinations() async {
    ...
    let combinations:
      [(accepted: Bool, blocked: Bool, rejected: Bool, expected: Set<SuggestionStatusEntity>)] = [
    ...
    for (accepted, blocked, rejected, expected) in combinations {
      ...
      // Then
      #expect(
        manager.optionalVisibleStatuses == expected,
        "Failed for combination: accepted=\(accepted), blocked=\(blocked), rejected=\(rejected)")
    }
  }
...
struct ConfigurationErrorTests {
  ...
  func testConfigurationErrorDescriptions() async {
    // Given/When/Then
    #expect(
      ConfigurationError.alreadyConfigured.errorDescription == "Votice SDK is already configured")
    #expect(
      ConfigurationError
        .notConfigured
